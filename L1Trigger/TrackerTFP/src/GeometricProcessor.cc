...
namespace trackerTFP {
  ...
  void GeometricProcessor::produce(const vector<vector<StubPP*>>& streamsIn,
                                   vector<deque<StubGP*>>& streamsOut,
                                   vector<deque<StubGP*>>& streamsTrunc) {
    ...
    for (int channelOut = 0; channelOut < numChannelOut; channelOut++) {
      ...
      // stream of lost stubs due to truncation
      deque<StubGP*>& truncated = streamsTrunc[channelOut];
  ...
  StubGP* GeometricProcessor::produce(const StubPP& stub, int phiT, int zT) {
    ...
    static const DataFormat& dfZ = dataFormats_->format(Variable::z, Process::gp);
    static const DataFormat& dfL = dataFormats_->format(Variable::layer, Process::gp);
    double baseCot = dfZ.base() / dfR.base();
    const int baseDiff = ceil(log2(2. * setup_->maxCot() / baseCot - 1e-12)) - setup_->widthDSPbb();
    baseCot *= pow(2., baseDiff);
    const double cot = (floor(dfZT.floating(zT) / setup_->chosenRofZ() / baseCot + 1e-12) + .5) * baseCot;
