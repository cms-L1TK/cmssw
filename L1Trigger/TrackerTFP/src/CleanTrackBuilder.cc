...
namespace trackerTFP {
  ...
  void CleanTrackBuilder::cleanTrack(
    ...
    static const int numBinsCot = setup_->ctbNumBinsZ0();
    ...
    static const double baseZTorig = dataFormats_->base(Variable::zT, Process::ht) / numBinsZT;
    static const double baseCotGP =
        (dataFormats_->base(Variable::zT, Process::gp) + 2. * setup_->beamWindowZ()) / setup_->chosenRofZ();
    static const double baseCot = baseCotGP / numBinsCot;
    static const double baseR = dataFormats_->base(Variable::r, Process::gp);
    static const double baseZorig = dataFormats_->base(Variable::z, Process::gp);
    static const double baseZT = pow(2., round(log2(baseZTorig / baseR / baseCot))) * baseR * baseCot;
    static const double baseZ = pow(2., round(log2(baseZorig / baseR / baseCot))) * baseR * baseCot;
    ...
    auto noTrack = [this, &maybePattern, zT](const TTBV& pattern) {
      ...
      if (pattern.count(0, setup_->kfMaxSeedingLayer()) < setup_->kfNumSeedStubs())
        return true;
      // pass
      return false;
    };
    ...
    for (StubHT* stub : track) {
      ...
      auto zT = [this, stub](double cot, double dZ) {
        ...
        const double z = (dfZ.integer(stub->z()) + .5) * baseZ;
        const double d = (dfZ.integer(dZ) + .5) * baseZ;
        return cot * r + z + d / 2.;
      };
      ...
      for (int binCot = 0; binCot < numBinsCot; binCot++) {
        ...
        const int binZTMin = floor(*min_element(zTs.begin(), zTs.end()) / baseZT + 1.e-12) + numBinsZT / 2;
        const int binZTMax = floor(*max_element(zTs.begin(), zTs.end()) / baseZT + 1.e-12) + numBinsZT / 2;
