...
namespace trackerTFP {
  ...
  Format<Variable::z, Process::gp>::Format(const Setup* setup) : DataFormat(true) {
    ...
    const double d = setup->innerRadius() * inv2R.range() / 2.;
    const double dPhi = asin(d) - d;
    const double dZ = dPhi / inv2R.range() * rangeCot;
    base_ = z.base();
    range_ = zT.base() + rangeCot * setup->maxRz() + dZ;
    width_ = ceil(log2(range_ / base_));
  }
  template <>
  ...
  Format<Variable::phi, Process::ht>::Format(const Setup* setup) : DataFormat(true) {
    ...
    const double d = setup->innerRadius() * inv2R.range() / 2.;
    const double dPhi = asin(d) - d;
    range_ = phiT.base() + setup->maxRphi() * inv2R.base() + dPhi;
    base_ = phi.base();
    width_ = ceil(log2(range_ / base_));
  }
  template <>
  ...
  Format<Variable::dPhi, Process::ctb>::Format(const Setup* setup) : DataFormat(false) {
    const Format<Variable::phi, Process::dtc> phi(setup);
    const Format<Variable::inv2R, Process::ctb> inv2R(setup);
    range_ = setup->pitchRowPS() / setup->innerRadius() +
             (setup->pitchCol2S() + setup->scattering()) * inv2R.range() / 2. + phi.base();
    base_ = phi.base();
    width_ = ceil(log2(range_ / base_));
  }
  template <>
  Format<Variable::dZ, Process::ctb>::Format(const Setup* setup) : DataFormat(false) {
    const Format<Variable::z, Process::dtc> z(setup);
    range_ = setup->pitchCol2S() * sinh(setup->maxEta()) + z.base();
    base_ = z.base();
    width_ = ceil(log2(range_ / base_));
  }
  template <>
  ...
  template <>
  Format<Variable::inv2R, Process::ctb>::Format(const Setup* setup) : DataFormat(true) {
    const Format<Variable::inv2R, Process::ht> inv2R(setup);
    base_ = inv2R.base();
    range_ = 2. * setup->invPtToDphi() / setup->minPtCand();
    width_ = ceil(log2(range_ / base_));
  }
  ...
  Format<Variable::phi, Process::kf>::Format(const Setup* setup) : DataFormat(true) {
    ...
    range_ = phi.range() * setup->kfRangeFactor();
    base_ = phi.base();
    width_ = ceil(log2(range_ / base_));
  }
  template <>
  ...
  Format<Variable::phiT, Process::kf>::Format(const Setup* setup) : DataFormat(true) {
    const Format<Variable::phiT, Process::dr> dr(setup);
  ...
  Format<Variable::cot, Process::kf>::Format(const Setup* setup) : DataFormat(true) {
    ...
    base_ = cot.base() * pow(2., setup->kfBaseShift());
    width_ = ceil(log2(range_ / base_));
  }
  template <>
  Format<Variable::zT, Process::kf>::Format(const Setup* setup) : DataFormat(true) {
    const Format<Variable::zT, Process::dr> dr(setup);
  ...
  Format<Variable::inv2R, Process::dr>::Format(const Setup* setup) : DataFormat(true) {
    const Format<Variable::inv2R, Process::ht> inv2R(setup);
    width_ = setup->tfpWidthInvR() + 1;
    range_ = inv2R.range() + 2. * inv2R.base();
    base_ = inv2R.base();
    const int shift = ceil(log2(range_ / base_)) - width_;
    base_ *= pow(2., shift);
  }
  ...
  Format<Variable::cot, Process::dr>::Format(const Setup* setup) : DataFormat(true) {
    ...
    base_ = zT.base() / phiT.base() * inv2R.base();
    const int shift = ceil(log2(range_ / base_)) - width_;
    base_ *= pow(2., shift);
  }
  template <>
