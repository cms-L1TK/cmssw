...
namespace trklet {
  ...
  void AnalyzerTBout::beginRun(const Run& iEvent, const EventSetup& iSetup) {
    ...
    for (Resolution r : AllResolution) {
      ...
      hisResolutionMe_.emplace_back(dir.make<TH1F>(("HisResMe" + name(r)).c_str(), ";", binsHis, -ranges[r], ranges[r]));
      hisResolutionThey_.emplace_back(dir.make<TH1F>(("HisResThey" + name(r)).c_str(), ";", binsHis, -ranges[r], ranges[r]));
      his2Resolution_.emplace_back(dir.make<TH2F>(("His2" + name(r)).c_str(), ";;", bins, -ranges[r], ranges[r], bins, -ranges[r], ranges[r]));
    }
  ...
  void AnalyzerTBout::analyze(const Event& iEvent, const EventSetup& iSetup) {
    ...
    for (int r : ttDTC.tfpRegions()) {
      for (int c : ttDTC.tfpChannels()) {
        ...
        copy_if(s.begin(), s.end(), back_inserter(regionStubs_[r]), [](const FrameStub& f){ return f.first.isNonnull(); });
      }
    }
    ...
    for (region_ = 0; region_ < setup_->numRegions(); region_++) {
      const int offset = region_ * channelAssignment_->numChannels();
      int nStubs(0);
      int nTracks(0);
      int nLost(0);
      for (int channel = 0; channel < channelAssignment_->numChannels(); channel++) {
        ...
        nTracks += tracks.size();
        nStubs += accumulate(tracks.begin(), tracks.end(), 0, [](int& sum, const auto& v){ return sum += (int)v.size(); });
        nLost += lost.size();
  ...
  void AnalyzerTBout::endJob() {
    ...
    log_ << "number of overflowed rz residuals: " << nOverflows_[0] << " and phi: " << nOverflows_[1] << endl;
    log_ << "=============================================================";
  ...
  void AnalyzerTBout::formTracks(const StreamsTrack& streamsTrack,
                                ...
                                vector<vector<TTStubRef>>& tracks,
                                int channel) {
    const int offset = channel * channelAssignment_->maxNumProjectionLayers();
    ...
    for (int frame = 0; frame < (int)streamTrack.size(); frame++) {
      ...
      if (frameTrack.first.isNull())
        continue;
      for (int layer = 0; layer < channelAssignment_->maxNumProjectionLayers(); layer++) {
        const FrameStub& stub = streamsStubs[offset + layer][frame];
        if (stub.first.isNonnull())
          this->fill(frameTrack, stub);
      }
      tracks.push_back(frameTrack.first->getStubRefs());
    }
  }
  ...
  void AnalyzerTBout::fill(const FrameTrack& frameTrack, const FrameStub& frameStub) {
    ...
    const auto it = find_if(region.begin(), region.end(), [&frameStub](const FrameStub& f){ return f.first == frameStub.first; });
    if (it == region.end())
