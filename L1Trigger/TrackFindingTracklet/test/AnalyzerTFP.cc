...
namespace trklet {
   ...
   *  \date   20204 Aug
   */
  class AnalyzerTFP : public one::EDAnalyzer<one::WatchRuns, one::SharedResources> {
  public:
    AnalyzerTFP(const ParameterSet& iConfig);
  ...
  AnalyzerTFP::AnalyzerTFP(const ParameterSet& iConfig) : useMCTruth_(iConfig.getParameter<bool>("UseMCTruth")) {
    usesResource("TFileService");
    // book in- and output ED products
    const string& label = iConfig.getParameter<string>("OutputLabelTFP");
    const string& branch = iConfig.getParameter<string>("BranchTTTracks");
    edGetToken_ = consumes<TTTracks>(InputTag(label, branch));
    if (useMCTruth_) {
  ...
  void AnalyzerTFP::beginRun(const Run& iEvent, const EventSetup& iSetup) {
  ...
  void AnalyzerTFP::analyze(const Event& iEvent, const EventSetup& iSetup) {
  ...
  // gets all TPs associated too any of the tracks & number of tracks matching at least one TP
  void AnalyzerTFP::associate(const vector<vector<TTStubRef>>& tracks,
                              const StubAssociation* ass,
                              set<TPPtr>& tps,
                              int& nMatchTrk,
                              bool perfect) const {
