...
namespace trklet {
  ...
  private:
    struct Track {
      Track(const FrameTrack& frame, const DataFormats* df) : ttTrackRef_(frame.first) {
        TTBV ttBV = frame.second;
        df->format(Variable::zT, Process::kf).extract(ttBV, zT_);
        df->format(Variable::cot, Process::dr).extract(ttBV, cot_);
        df->format(Variable::phiT, Process::kf).extract(ttBV, phiT_);
        df->format(Variable::inv2R, Process::kf).extract(ttBV, inv2R_);
        d0_ = ttTrackRef_->d0();
      }
    ...
    //
    void associate(const vector<Track>& tracks,
  ...
  AnalyzerKF::AnalyzerKF(const ParameterSet& iConfig)
      : useMCTruth_(iConfig.getParameter<bool>("UseMCTruth")), nEvents_(0), hisRes_(5), profRes_(5) {
  ...
  void AnalyzerKF::analyze(const Event& iEvent, const EventSetup& iSetup) {
    ...
    for (int region = 0; region < setup_->numRegions(); region++) {
      ...
      for (int frame = 0; frame < (int)channelTracks.size(); frame++) {
        ...
        const double eta = abs(asinh(tracks.back().cot_));
        int nLayers(0);
  ...
  //
  void AnalyzerKF::associate(const vector<Track>& tracks,
    ...
    for (int frame = 0; frame < (int)tracks.size(); frame++) {
      const Track& track = tracks[frame];
      ...
      if (his.empty())
        continue;
      const double cot = track.cot_;
      const double z0 = track.zT_ - setup_->chosenRofZ() * cot;
      const double inv2R = track.inv2R_;
      const double phi0 = deltaPhi(track.phiT_ - setup_->chosenRofPhi() * inv2R +
      ...
      for (const TPPtr& tpPtr : tpPtrs) {
        ...
        const double dD0 = tpPtr->d0() + track.d0_;
