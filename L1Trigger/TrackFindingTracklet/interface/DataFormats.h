...
namespace trklet {
  ...
  private:
    ...
    static constexpr std::array<std::initializer_list<Variable>, +Process::end> stubs_ = {{
        {Variable::stubId, Variable::r, Variable::phi, Variable::z, Variable::dPhi, Variable::dZ},  // Process::tm
  ...
  class StubTM : public Stub<int, double, double, double, double, double> {
  public:
    ...
    StubTM(const TTStubRef& ttStubRef,
           ...
           double dPhi,
           double dZ)
        : Stub(ttStubRef, df, Process::tm, stubId, r, phi, z, dPhi, dZ) {}
    ~StubTM() {}
    // stub Id
  ...
  public:
    ...
    StubDR(const StubTM& stub) : Stub(stub, stub.r(), stub.phi(), stub.z(), stub.dPhi(), stub.dZ()) {}
    ~StubDR() {}
    // stub radius wrt chosenRofPhi
