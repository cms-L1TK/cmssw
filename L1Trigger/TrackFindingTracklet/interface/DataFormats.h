...
namespace trklet {
  ...
  private:
    ...
    static constexpr std::array<std::initializer_list<Variable>, +Process::end> tracks_ = {{
        ...
        {Variable::chi20, Variable::chi20, Variable::mva, Variable::hitPattern},  // Process::tq
        {}                                                                        // Process::tfp
    }};
  ...
  class TrackTQ : public Track<double, double, int, TTBV> {
  public:
    ...
    TrackTQ(const TrackKF& track, double chi20, double chi21, int mva, const TTBV& hitPattern)
        : Track(track, chi20, chi21, mva, hitPattern) {}
    TrackTQ() {}
    ~TrackTQ() override = default;
    // track r-phi chi2
    double chi20() const { return std::get<0>(data_); }
    // track r-z chi2
    double chi21() const { return std::get<1>(data_); }
    // mva
    int mva() const { return std::get<2>(data_); }
    // mva
    const TTBV& hitPattern() const { return std::get<3>(data_); }
  };
