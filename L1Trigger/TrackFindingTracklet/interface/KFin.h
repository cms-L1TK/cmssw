...
namespace trklet {
  ...
  private:
    ...
    struct Stub {
      Stub(const TTStubRef& ttStubRef, double r, double phi, double z, int layerId, bool psTilt, int channel) : ttStubRef_(ttStubRef), r_(r), phi_(phi), z_(z), layerId_(layerId), psTilt_(psTilt), channel_(channel) {}
    ...
    struct Track {
      ...
      Track(const tt::FrameTrack& frame, const std::vector<Stub*>& stubs, double cot, double zT, double inv2R, int sectorEta) : frame_(frame), stubs_(stubs), cot_(cot), zT_(zT), inv2R_(inv2R), sectorEta_(sectorEta) {}
