...
namespace trklet {
  ...
  void KalmanFilterFormats::consume(const DataFormats* dataFormats, const ConfigKF& iConfig) {
    iConfig_ = iConfig;
  ...
  void DataFormatKF::updateRangeActual(double d) {
    ...
    if (enableIntegerEmulation_ && !inRange(d)) {
      ...
      exception.addContext("trackFindingTracklet:DataFormatKF::updateRangeActual");
