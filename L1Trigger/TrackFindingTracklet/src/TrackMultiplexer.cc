...
namespace trklet {
  ...
  TrackMultiplexer::TrackMultiplexer(const ParameterSet& iConfig,
    ...
    const int unusedMSBScot = floor(log2(baseUcot_ * pow(2.0, channelAssignment_->widthCot()) / 2. / setup_->maxCot()));
    const int baseShiftScot = channelAssignment_->widthCot() - unusedMSBScot - 1 - setup_->widthAddrBRAM18();
    ...
    for (int channel = 0; channel < channelAssignment_->numChannelsTrack(); channel++) {
      ...
      for (int frame = 0; frame < (int)streamTrack.size(); frame++) {
        ...
        if (useTTStubParameters_) {
          ...
          const double H1m0 = (gp1.perp() - setup_->chosenRofPhi()) * deltaPhi(gp0.phi() - offset);
          const double H0m1 = (gp0.perp() - setup_->chosenRofPhi()) * deltaPhi(gp1.phi() - offset);
        ...
        for (int seedingLayer = 0; seedingLayer < channelAssignment_->numSeedingLayers(); seedingLayer++) {
          ...
          if (barrel) {
            ...
            const double z = z0 + (r + setup_->chosenRofPhi()) * cot;
            const double dZ = abs(z) - setup_->tbBarrelHalfLength();
            if (dZ > 0.)
              r -= dZ / abs(cot);
          } else {
            const double side = zT < 0. ? -1. : 1.;
            const double disk =
                digi(setup_->hybridDiskZ(layerId - setup_->offsetLayerId() - setup_->offsetLayerDisks()), baseUzT_);
            const double invCot = digi(1. / digi(cot, baseScot_), baseInvCot_);
            r = (side * disk - z0) * invCot;
        ...
        if (useTTStubResiduals_) {
          for (Stub* stub : stubs) {
            ...
            stub->phi_ = deltaPhi(gp.phi() - region_ * setup_->baseRegion());
        ...
        // stub range checks
        for (Stub* stub : stubs) {
          if (!dataFormats_->format(Variable::phi, Process::ctb).inRange(stub->phi_, true))
            stub->valid_ = false;
          if (!dataFormats_->format(Variable::z, Process::ctb).inRange(stub->z_, true))
            stub->valid_ = false;
        }
        // layer check
  ...
  void TrackMultiplexer::produce(StreamsTrack& streamsTrack, StreamsStub& streamsStub) {
    ...
    for (Track& track : tracks_) {
      ...
      if (hits.count() < setup_->kfMinLayers())
        track.valid_ = false;
      if (hits.count(0, setup_->kfMaxSeedingLayer()) < setup_->kfNumSeedStubs())
        track.valid_ = false;
    }
    ...
    for (deque<Track*>& stream : streams)
    ...
    auto frameStub = [this](Track* track, int layer) {
      ...
      if (!track->valid_ || it == track->stubs_.end() || !(*it)->valid_)
        return FrameStub();
      Stub* stub = *it;
      const TTBV stubId(stub->stubId_, channelAssignment_->widthStubId(), false);
  ...
  template <class T>
  T* TrackMultiplexer::pop_front(deque<T*>& ts) const {
    T* t = nullptr;
    if (!ts.empty()) {
      t = ts.front();
      ts.pop_front();
