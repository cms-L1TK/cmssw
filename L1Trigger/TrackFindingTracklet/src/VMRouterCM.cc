...
void VMRouterCM::printStub(const Stub* stub){
  ...
  globals_->ofstream("stubposition.txt") << layerdisk_ << " "
					 << phi << " " <<stub->l1tstub()->phi() << " "
					 << settings_.kphi(layerdisk_) << " " 
					 << r << " " << stub->l1tstub()->r() << " "
    					 << ((layerdisk_>=N_LAYER && (!stub->isPSmodule())) ? -1.0:settings_.kr(layerdisk_)) << " " 
					 << z << " " << stub->l1tstub()->z() << " "
    					 << settings_.kz() << std::endl;  
}
