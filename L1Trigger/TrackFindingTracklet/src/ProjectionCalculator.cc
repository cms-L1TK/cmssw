...
void ProjectionCalculator::addOutput(MemoryBase* memory, string output) {
  ...
  if (output == "projout") {
    ...
    unsigned int layerdisk =
        memory->getName()[memory->getName().size() - 5 - extraMPOffset] - '1';  //layer or disk counting from 0
    unsigned int phiregion =
        memory->getName()[memory->getName().size() - 1 - extraMPOffset] - 'A';  //phiregion counting from 0
    if (memory->getName()[memory->getName().size() - 6 - extraMPOffset] == 'D')
      layerdisk += N_LAYER;
    ...
    for (int iSeed = 0; iSeed < 8; ++iSeed) {
      ...
      if (seed == seedNames[iSeed]) {  // FIXME find easier way to get iSeed (probably from seed name)
        ...
        for (unsigned int k = 0; k < inputpars_[i]->nTracklets(); k++) {
          ...
          /////////////////////////////////
          bool valid_zmin, valid_zmax, valid_phimin,
              valid_phimax;  // CODE ADAPTED FROM FIRMWARE-HLS PROJECTIONCALCULATOR - could be simplified to only consider layers/disks in wiring config.
          ...
          for (unsigned int j = 0; j < settings_.projlayers()[iSeed].size(); ++j) {
            ...
            if (valid_LD[layer - 1]) {  // If projection to layer/disk is valid
              if ((izr_LD[layer - 1] == -(1 << (settings_.nzbitsstub(layer - 1) - 1))) ||
                  (izr_LD[layer - 1] ==
          ...
          for (unsigned int j = 0; j < settings_.projdisks()[iSeed].size(); ++j) {
            ...
            if (valid_LD[N_LAYER + disk - 1]) {  // If projection to layer/disk is valid
              ...
              if (iSeed <= 4) {  // if barrel seed, need to check if haven't already projected to layer
                if (disk == 1 && addedLayer[5])
                  continue;
                if (disk == 2 && addedLayer[4])
                  continue;
                if (disk == 3 && addedLayer[3])
                  continue;
                if (disk == 4 && addedLayer[2])
                  continue;
              }
