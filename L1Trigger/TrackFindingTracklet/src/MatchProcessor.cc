...
void MatchProcessor::execute(unsigned int iSector, double phimin) {
  ...
  */
  ...
  for (unsigned int istep = 0; istep < settings_.maxStep("MP"); istep++) {
    ...
    if (print) {
      std::cout << "istep = "<<istep<<" projBuff: "<<inputProjBuffer_.rptr()<<" "<<inputProjBuffer_.wptr()<<" "<<projBufferNearFull;
	...
	int rindex =
	  (projdata_->proj(layerdisk_).fpgarzproj().value() >> (projdata_->proj(layerdisk_).fpgarzproj().nbits() - nrbits_)) &
	  ((1 << nrbits_) - 1);
    ...
    if (validin) {
      ...
      if (print & validin) {
	std::cout << "Reading iprojmem page, readaddress : " << istep << " " << imem << " " << ipage << " " << read_address << std::endl;
      }
    }
