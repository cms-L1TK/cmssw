...
namespace HPH {
    ...
    deltaTanL_(Setup_->deltaTanL())
    {
     ...
     for (SensorModule sm : Setup_->SensorModules()) {
         ...
         if (!(abs(d_p) < sm.numColumns() * sm.pitchCol() / 2. && abs(d_m) < sm.numColumns() * sm.pitchCol() / 2.)) sm.setMaybe();
     ...
     layers_.erase( unique( layers_.begin(), layers_.end(), equalID ), layers_.end() );//Only keep sensor modules that look unique in R-Z plane
     ...
     if (HPHdebug_){
        ...
        edm::LogVerbatim("TrackTriggerHPH")<<"Looking at hitpattern "<<std::bitset<7>(hitpattern_)
                                           <<"; Loop over KF layers:";
     }
     ...
     if (useNewKF_){
      ...
      for (int i=0;i<7;i++){//Loop over each digit of hitpattern
          ...
          for (int j : hitmap_[kf_eta_reg][i]){//Find out which layer the Old KF is dealing with when hitpattern is encoded
              if (j<1) {
              ...
              edm::LogVerbatim("TrackTriggerHPH")<<"Rejected by sensor modules"<<j;
              }
     ...
     if (HPHdebug_){
     edm::LogVerbatim("TrackTriggerHPH")<<"------------------------------";
     edm::LogVerbatim("TrackTriggerHPH")<<"numPS = "<<numPS_<<", num2S = "<<num2S_<<", missingPS = "<<numMissingPS_<<", missing2S = "<<numMissing2S_;
     edm::LogVerbatim("TrackTriggerHPH")<<"======================================================";
     }
