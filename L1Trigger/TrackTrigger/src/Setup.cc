...
namespace tt {
  ...
  void Setup::calculateConstants() {
    ...
    for (int type = 0; type < SensorModule::NumTypes; type++)
      if (type == SensorModule::DiskPS) hybridBasesR_.emplace_back(hybridRangesR_.at(type) / pow(2., hybridWidthsR_.at(type) + 1)); // to account for 7.5 cm offset 
      else hybridBasesR_.emplace_back(hybridRangesR_.at(type) / pow(2., hybridWidthsR_.at(type)));
    ...
    for (int type = 0; type < SensorModule::NumTypes; type++)
      hybridNumsUnusedBits_.emplace_back(TTBV::S_ - hybridWidthsND_.at(type) - hybridWidthsR_.at(type) - hybridWidthsZ_.at(type) -
